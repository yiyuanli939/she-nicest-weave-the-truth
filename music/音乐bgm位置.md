...
| level_1 | 第一章 并纹:进关前故事界面 + 关内 | (待补) | |
| level_2 | 第二章 叠层纹:故事界面 + 关内 | (待补) | |
| level_3 | 第三章 岔纹:故事界面 + 关内 | (待补) | |
| level_4 | 第四章 焦纹:故事界面 + 关内 | (待补) | |

## 补曲 / 换曲

1. 文件放到 `music/<槽位>.mp3`(换曲直接覆盖同名文件;两章共用一首就在 `TRACKS` 里填同一个路径)。
