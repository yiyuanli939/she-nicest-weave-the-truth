...
| `ui/settings_panel.gd` | 标题页设置模块(四个选项正下方):音乐音量滑条 / 全屏 / 小机联动(= 无机器人模式开关)/ 小机维护;落 `SaveManager.settings`(重置进度不清),启动时 `Bgm._ready` 读音量、`Game._apply_window_settings` 恢复全屏 |
...
不连桥接不轮询;切回入口:标题页设置模块「小机联动」/ F9 面板,所有构建);守门 `tests/test_perf_settings.gd` + UI smoke S/T 节)✅ →
...
**标题页设置模块**(2026-09-02,用户要求、美术文档没有 → 纯文字 + 常量留位:`ui/settings_panel.gd` 挂在四个选项正下方同列居中,
音乐音量滑条(`Bgm.user_volume`,当场生效)/ 全屏开关(`DisplayServer` 窗口模式,启动恢复)/ 小机联动开关(= 无机器人模式,Web 不显示)/ 小机维护入口;
