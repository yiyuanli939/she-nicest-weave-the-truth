...
- 原版无独立否定规则:**¬A ≡ A→⊥**,只有 `falseE`(爆炸原理)与经典章节的 `TND`(排中律)。
...
1. **收集方程**:每条边一条 `formula(from口) ≐ formula(to口)`;每个 pinned 假设一条 `?k ≐ 玩家纹样`。
2. **合一**:`Unifier.solve(eqs)` → subst + conflicts(冲突边标 `CONFLICT`)。
...
var solved: bool
```
`UNDERSPEC` = 该边 resolve 后仍含 META("?"徽章,不算硬错误但阻断胜利)。
...
- **第五章·两仪**(可选,+tnd):`⊢ A∨(A→⊥)`;`A→B ⊢ (A→⊥)∨B`

叙事挂点:第一~二章工坊学艺 → 第三~四章档案室发现同构(笔记本揭示)→ 第五章机器人失控/错误前提主线。
...
| 机器徽记 | 封程/引渡/并织/拆股/岔纹/汇路/溃散/两仪 + 线轴(假设)+ 目标织机 | 10 |
