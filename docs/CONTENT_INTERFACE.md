...
| 列 | 取值 |
|---|---|
| 关卡 | `1-1` … `4-3` 是源表的原剧情段号,播放位置按下表调整;或用 `l01` … `l16` 直接指定关前剧情。**原 `4-3` 结局不动**:写进 l16 的 `outro_dialogue`,通关弹窗点「继续」才播,播完「感谢游玩」黑屏 → 开发者信息页。导入会重写源关卡与目标关卡的 intro/outro(缺的一侧清空) |
...
**2026-09-16 剧情编排**:源 xlsx / CSV 段号不改,`tools/import_dialogue.gd` 的 `EPISODE_PLACEMENT` 统一控制播放位置,重新导入仍保留新编排。

| 原剧情段号 | 新播放位置 |
|---|---|
| 2-1 | 接在第一章第五关(l05)原开场剧情后,进关前连续播放;源表中保持 1-5 在 2-1 前 |
| 2-2 | 第二章第一关(l06)前;第二关(l07)直接进棋盘 |
| 4-1 | 第四章第二关(l15)前;第一关(l14)直接进棋盘 |
| 4-2 | 第四章第三关(l16)前 |
| 4-3 | 第四章第三关(l16)通关后,结局不动 |

非法的场景/人物/表情整行跳过并报错,**有任何错误一关都不写**;表里未涉及的源关卡及目标关卡不动。可用角色/表情/场景在 `narrative/story_art.gd`。
...
- `robot_cue_on_enter` / `robot_cue_on_win`:进关/通关的小机演出。现在进关一律为空;通关一律 `celebrate`,只有 l11(3-1)是 `panic`——
  ...
  **结局才修好**——l16 通关后 4-3 剧情播完的「感谢游玩」黑屏时 `calm`(在 `ui/story_scene.gd`)。
  `allow_bot`:纹样编辑器是否解锁焦纹笔刷(第四章起 on;v1.2 起笔刷是焦纹图样,不是文字)。
- 小机「请指导我 / 请帮帮我」(玩家对麦克风说):坏掉前(l01–l11)小机回头到极限后**直接代解本关**(不庆祝不鼓励),坏掉后只故障演出。
  关内提示文案在 `ui/level_scene.gd` `GUIDE_HINT`(坏掉前才显示;**无机器人模式**下不显示,见 docs/ROBOT_API.md「无机器人模式」);语音识别与小机维护见 docs/ROBOT_API.md。
- **设计关卡时注意求解是严格正向的**:仪器输出只由输入 + 玩家钉的纹样决定,不会从目标反推。
  用到封程机(假设 P)、岔纹机(另一支)、溃散机(织出什么)的关,玩家必须点该口纹样旁的「钉纹样」按钮
  (弹「纹样绘制」窗;未钉的口纹样外有一圈虚线)给自由纹样赋值;关卡 `atoms` 要包含玩家需要钉的原子。
  脚本化解法在 `levels/level_solutions.gd` 的 `p` 数组。
- **接错的线自己会断**(v1.1):冲突 / 成环 / 假设逃逸的线接上 0.5 s 后自动断开,错误字提示停 1 s 淡出;
  「欠定」(还有未染纱)不是接错,线留着等玩家钉。依赖未消去假设的线整条是假设色(红棕),
  提醒玩家它不能直接接进目标织机。
...
棋盘左下(求助提示上一行)按棋盘状态提示玩家下一步**操作**,三条,按优先级取第一条还没做过的:
有口没钉时点钉纹样(pin)→ 空盘从仪器架放仪器(place)→ 盘上有机没线时拉线(wire)。
(v1.1 起不再提示断线拆机与翻笔记:接错的线自己断开、新仪器的笔记进关自动弹出。)
每条做过一次就记进存档 `steps`,以后各关都不再提示;「重置进度」清掉。文案在 `narrative/step_guide.gd` `TEXT`(只讲操作,
同笔记的文案守则:不写逻辑符号/术语/解法),位置字号颜色在 `ui/level_scene.gd` `STEP_HINT_*`(美术之后换图或挪位改那里)。
